...
background-image: url('<?php echo $profpic;?>');
	...
	<title>Book a Group Study Room</title>
	<style type="text/css">
		body {
...
<body>
	...
	<form action="http://localhost/real_library/bookroom/book.php" method="post">
		...
		$cat = $_GET['cat'];
	echo "<select id='room_type' onchange='reload()' name='room_type'>";

	if ($cat == 'I'){

		echo "<option value='I' selected  >Individual Study Room</option>";
		echo "<option value='G'>Group Study Room</option>";
	}
	else{
		echo "<option value='I'  >Individual Study Room</option>";
		echo "<option value='G' selected >Group Study Room</option>";
	}
        echo "</select>";
		?>
			
		<label for="room_no">Select a Room Number:</label>
        <?php

$cat = $_GET['cat'];
$query2 = "select room_no, capacity from study_rooms where room_type =?";
